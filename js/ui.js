...
export class UIRenderer {
  ...
  renderHome(sets, searchQuery = "", sortBy = "newest") {
    const setsHTML = sets
      .map(
        (set) => `
        <div class="card">
            <div class="card-header">
                <div style="flex: 1;">
                    ...
                    ${
                      set.lastPlayedAt
                        ? `
                        <p class="card-last-played" style="margin-top: 0.25rem; font-size: 0.875rem; color: #6b7280;">
                            Грали: ${this.formatLastPlayed(set.lastPlayedAt)}
                        </p>
                    `
                        : ""
                    }
                ...
                <div style="display: flex; gap: 0.5rem;">
                    <button class="btn-icon blue" data-action="export" data-id="${
                      set.id
                    }">
                        ${icons.upload}
                    </button>
                    <button class="btn-icon blue" data-action="edit" data-id="${
                      set.id
                    }">
                        ${icons.edit}
                    </button>
                    <button class="btn-icon red" data-action="delete" data-id="${
                      set.id
                    }">
                        ${icons.trash}
                    </button>
            ...
            <button class="btn btn-gradient" style="width: 100%;" data-action="play" data-id="${
              set.id
            }">
                ${icons.play}
                Почати гру
    ...
    this.appElement.innerHTML = `
        <div class="gradient-bg-purple" style="min-height: 100vh;">
            <div class="container">
                ...
                <div class="desktop-action-buttons" style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
                ...
                <div class="search-sort-section">
                    <div class="search-box">
                        <div class="search-input-wrapper">
                            ...
                            ${
                              searchQuery
                                ? `
                                <button class="search-clear" data-action="clear-search">
                                    ${icons.x}
                                </button>
                            `
                                : ""
                    ...
                    <div class="sort-box">
                        <select class="input sort-select" data-action="sort" style="margin-bottom: 0;">
                            ...
                            <option value="oldest" ${
                              sortBy === "oldest" ? "selected" : ""
                            }>Найстаріші</option>
                            <option value="recent" ${
                              sortBy === "recent" ? "selected" : ""
                            }>Нещодавно грані</option>
                            ...
                            <option value="cards-asc" ${
                              sortBy === "cards-asc" ? "selected" : ""
                            }>Карток менше</option>
                            <option value="cards-desc" ${
                              sortBy === "cards-desc" ? "selected" : ""
                            }>Карток більше</option>
                        </select>
                    </div>
                </div>
                      ...
                      sets.length > 0
                        ...
                        <div class="empty-state">
                            ${
                              searchQuery
                                ? `
                                <p style="font-size: 1.25rem;">Нічого не знайдено</p>
                                <p style="opacity: 0.8; margin-top: 0.5rem;">Спробуйте інший запит</p>
                            `
                                : `
  ...
  renderEdit(editingSet) {
    const cardsHTML = editingSet.cards
      .map(
        (card, index) => `
        <div class="card-edit-item">
            ...
            <input type="text" class="input" placeholder="Запитання" value="${
              card.question
            }" data-card-id="${
          card.id
        }" data-field="question" style="margin-bottom: 0.5rem;">
            <input type="text" class="input" placeholder="Відповідь" value="${
              card.answer
            }" data-card-id="${card.id}" data-field="answer">
        </div>
    `
      )
      .join("");
        ...
        <div class="gradient-bg-blue" style="min-height: 100vh;">
            <div class="container container-small">
                ...
                <div class="card">
                    ...
                    <input type="text" class="input input-lg" placeholder="Назва набору" value="${
                      editingSet.name
                    }" data-field="set-name" style="margin-bottom: 1rem;">
  ...
  renderGame(gameState, selectedSet) {
    ...
    if (!gameState || !selectedSet) {
      return this.renderError("Помилка завантаження гри");
    }
    ...
    if (!currentCard) {
      return this.renderError("Картка не знайдена");
    }

    // Calculate progress
    const progress = Math.round(
      ((gameState.currentIndex + 1) / gameState.cards.length) * 100
    );
                        ...
                        <div class="card result-card-mobile" style="padding: 2rem;">
                            ...
                            <div style="text-align: center; margin-bottom: 1.5rem;">
                                ...
                                <p style="color: #6b7280; font-size: 1.125rem;">
                                    Правильно: ${gameState.correctCount} з ${
      gameState.cards.length
    }
                                </p>
                            </div>
                                ...
                                <div class="improvement">
                                    <p style="font-size: 1.125rem; color: #374151;">
                                        ...
                                        `
                                            : improvement < 0
                                            ? `
                                            <span class="improvement-negative">↓ Зниження на ${Math.abs(
                                              improvement
                                            )}%</span>
                                        `
                                            : `
  ...
  renderGenerateModal() {
    ...
    return document.getElementById("generate-modal");
  }
